...
export default function Collection({ collection }: CollectionProps) {
  ...
  if (collection.nft != null) {
    return <>
      ...
      <div className="p-5">
          <a href={"/my_collections/" + collection.id}>
              <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{collection.title}</h5>
          </a>
          <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{moment.unix(Number(collection.created_time)).format("yyy-MM-DD hh:mm:ss")}</p>
          <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">{collection.description}</p>
          <a href={"/my_collections/" + collection.id} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
              Read more
  ...
  return <>
    <div className="m-auto min-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
      <div className="flex flex-row items-center justify-center">
        <Image width={200} height={200} src={collection.icon_url} alt={''}/>
        <Link href={"/launching_nft/" + collection.id}>
          发行NFT
        </Link>
      </div>
      <div className="p-5">
